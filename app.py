...
def init():
    ...
    if not os.path.exists(ARQ_DADOS):
        lista=[{"ID":1,"NOME":"CIMENTO","UNIDADE":"SC","MARCA":"VOTORANTIM","LOCAL":"BARRACÃO","SALDO":0},{"ID":1,"NOME":"CIMENTO","UNIDADE":"SC","MARCA":"VOTORANTIM","LOCAL":"OFICINA","SALDO":0}]
...
st.title(f"📦 TOTAL: {pivot['TOTAL GERAL'].sum():.0f}")
...
# REGISTROS - CORRIGIDO PARA NAO DAR KEYERROR
if st.session_state.mov and len(st.session_state.mov)>0:
    df_mov=pd.DataFrame(st.session_state.mov)
    if "IDX" in df_mov.columns and not df_mov.empty:
        df_mov=df_mov.sort_values("IDX", ascending=False)
        st.divider()
        st.subheader("📋 Registros")
        st.dataframe(df_mov, use_container_width=True)
        id_del=st.selectbox("IDX apagar", df_mov["IDX"].tolist())
        if st.button("🗑️ APAGAR E ESTORNAR"):
            ...
            if mov_del:
                ...
                pd.DataFrame(st.session_state.dados).to_csv(ARQ_DADOS,index=False)
                pd.DataFrame(st.session_state.mov).to_csv(ARQ_MOV,index=False) if st.session_state.mov else pd.DataFrame(columns=["IDX","DATA_HORA","LOTE","MARCA","FORNECEDOR","VALIDADE","QTD_PALETE","ENTRADA","TOTAL","UNIDADE","LOCAL","TIPO","ID_MAT","NOME_MAT","RESPONSAVEL","OBS"]).to_csv(ARQ_MOV,index=False)
