...
st.set_page_config(page_title="Almox Validade Editável", layout="wide")
...
def init():
    if not os.path.exists(ARQ_DADOS):
        lista=[{"ID":1,"NOME":"CIMENTO","UNIDADE":"SC","MARCA":"-","LOCAL":"BARRACÃO","SALDO":0,"VALIDADE_PADRAO":90},{"ID":1,"NOME":"CIMENTO","UNIDADE":"SC","MARCA":"-","LOCAL":"OFICINA","SALDO":0,"VALIDADE_PADRAO":90}]
    ...
    if not os.path.exists(ARQ_FORN):
        pd.DataFrame([{"ID_FORN":i+1,"NOME":n,"MARCA":n} for i,n in enumerate(["FONDU","SHINAGAWA","TECNOFIRE","CABOFRAX","IBAR","BIOLÃ"])]).to_csv(ARQ_FORN,index=False)
...
with st.sidebar.expander("✏️ EDITAR VALIDADE DE LOTE JÁ LANÇADO", expanded=False):
    if st.session_state.mov:
        ...
        try: fab_atual=datetime.strptime(lote_sel['DATA_FAB'],"%d/%m/%Y").date()
        except: fab_atual=hoje
        try: val_atual=datetime.strptime(lote_sel['VALIDADE'],"%d/%m/%Y").date()
        except: val_atual=hoje+timedelta(days=180)
        ...
        if st.button("💾 Atualizar lote"):
            ...
            st.success("Lote atualizado"); st.rerun()
...
pivot["TOTAL"]=pivot["BARRACÃO"]+pivot["OFICINA"]
...
id_sel=st.selectbox("Material", ids, format_func=lambda x: f"{x} - {mapa[x][0]} - Padrão {mapa[x][2]} dias")
