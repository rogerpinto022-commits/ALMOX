...
st.set_page_config(page_title="REFORMA FORNOS V11 OK", layout="wide")
...
if not st.session_state.logado:
    ...
    e = st.text_input("Email", key="login_email_v11")
    s = st.text_input("Senha", type="password", key="login_senha_v11")
    if st.button("Entrar", key="btn_entrar_v11"):
...
st.title(f"🔥 REFORMA FORNOS V11 - {agora.strftime('%d/%m/%Y %H:%M')}")

tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["🖥️ DASHBOARD","📝 CADASTRO","🔄 ENTRADA/SAIDA","📦 ESTOQUE","📊 BUSCA ID","📦 GRD","📈 GRAFICOS"])
...
with tab2:
    ...
    id_b = st.text_input("DIGITE ID PARA RECONHECER", key="cad_id_busca_v11")
    desc_def=""; marca_def=""
    for r in st.session_state.cad:
        if str(r.get('ID','')).upper()==id_b.upper():
            desc_def=r.get('DESCRICAO',''); marca_def=r.get('MARCA','')
    with st.form("form_cad_v11"):
        id_in=st.text_input("ID*", value=id_b.upper() if id_b else "1", key="cad_id_v11")
        desc_in=st.text_input("DESCRIÇÃO*", value=desc_def, key="cad_desc_v11")
        marca_in=st.text_input("MARCA*", value=marca_def, key="cad_marca_v11")
        lote_in=st.text_input("LOTE OPCIONAL", key="cad_lote_v11")
        local_in=st.selectbox("LOCAL", LOCAIS, key="cad_local_v11")
        qtd_in=st.number_input("QTD/PALETE", value=1250.0, key="cad_qtd_v11")
        ent_in=st.number_input("QTD PALETES", value=0.0, key="cad_ent_v11")
    ...
    if st.session_state.cad:
        for i,r in enumerate(st.session_state.cad):
            ...
            with c3:
                if st.button("Excluir", key=f"del_cad_v11_{i}"):
...
with tab3:
    ...
    if not ids: st.warning("Cadastre ID primeiro")
        ...
        id_sel=st.selectbox("ID*", options=sorted(ids), key="mov_id_v11")
        cat=None
        for r in st.session_state.cad:
            if r.get('ID')==id_sel: cat=r
        desc=cat.get('DESCRICAO','') if cat else ""
        st.text_input("Descrição", value=desc, disabled=True, key="mov_desc_v11")
        lote=st.text_input("LOTE* OBRIGATÓRIO", key="mov_lote_v11")
        marca=st.text_input("MARCA*", value=cat.get('MARCA','') if cat else "", key="mov_marca_v11")
        local_o=st.selectbox("ORIGEM", LOCAIS, key="mov_orig_v11")
        tipo=st.selectbox("TIPO", ["ENTRADA","SAIDA","TRANSFERENCIA"], key="mov_tipo_v11")
        pal=st.number_input("QTD PALETES", value=1.0, key="mov_pal_v11")
        dest=st.selectbox("DESTINO TRANSFER", LOCAIS, index=1, key="mov_dest_v11")
        if st.button("CONFIRMAR", type="primary", key="mov_btn_v11"):
...
with tab5:
    ...
    id_b=st.text_input("DIGITE ID", key="busca_id_v11")
    if id_b:
        saldos=get_saldos()
...
with tab6:
    ...
    ids=list(set([r.get('ID','') for r in st.session_state.cad if r.get('ID')]))
    if ids:
        id_g=st.selectbox("ID GRD*", options=sorted(ids), key="grd_id_v11")
        cat=None
        for r in st.session_state.cad:
            if r.get('ID')==id_g: cat=r
        desc=cat.get('DESCRICAO','') if cat else ""
        st.text_input("Descrição GRD", value=desc, disabled=True, key="grd_desc_v11")
        lote=st.text_input("LOTE GRD*", key="grd_lote_v11")
        marca=st.text_input("MARCA GRD*", value=cat.get('MARCA','') if cat else "", key="grd_marca_v11")
        qtd=st.number_input("QTD PALETES GRD*", value=1.0, key="grd_qtd_v11")
        ori=st.selectbox("ORIGEM GRD*", LOCAIS, key="grd_ori_v11")
        dst=st.selectbox("DESTINO GRD*", [l for l in LOCAIS if l!=ori], key="grd_dst_v11")
        os_g=st.text_input("OS/FORNO*", key="grd_os_v11")
        if st.button("GERAR GRD", type="primary", key="grd_btn_v11"):
            if not lote: st.error("LOTE obrigatório")
            else:
...
with tab7:
    st.subheader("📈 GRAFICOS - SEM ERRO")
    saldos=get_saldos()
    ...
    if df.empty: st.info("Sem estoque")
        ...
        # GRAFICOS SEM text='TEXTO' para nao quebrar
        df_top = df.groupby('ID', as_index=False)['SALDO'].sum().sort_values(by='SALDO', ascending=False).head(20)
        fig = px.bar(df_top, x='ID', y='SALDO', title="TOP 20 IDs POR QTD")
        st.plotly_chart(fig, use_container_width=True)
...
st.caption(f"V11 FINAL OK - TODAS ABAS FUNCIONANDO - {agora.strftime('%d/%m/%Y %H:%M')}")
