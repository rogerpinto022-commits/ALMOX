...
# INICIALIZAÇAO SEM CONFLITO DE NOMES - CORRIGE StreamlitValueAssignmentNotAllowedError
...
st.markdown("<h1 style='text-align:center; background:#000; color:#00ff66; padding:18px; border-radius:12px; border:4px solid #ff4e00; font-family:Arial Black;'>🔥 REFORMA DE FORNOS - CONTROLE COMPLETO 🔥</h1>", unsafe_allow_html=True)

tab1,tab2,tab3,tab4,tab5=st.tabs(["📝 CADASTRO COMPLETO","🔄 ENTRADA/SAIDA","📦 ESTOQUE COMPLETO","📊 LOTES DO PRODUTO - BARRAS VALIDADE","📈 GRAFICOS"])

with tab1:
    st.markdown("#### CADASTRO COM FABRICAÇÃO, TEMPO VALIDADE, VÁLIDO ATÉ")
    with st.form("form_cadastro_principal", clear_on_submit=True):
        ...
        with c4:
            ...
            st.metric("TOTAL", f"{total_prev:,.0f} KG")
            data_cad=date.today()
        if st.form_submit_button("💾 CADASTRAR COMPLETO", type="primary", use_container_width=True):
            ...
            st.success(f"✅ LOTE {lote_in} | FAB {fab_str} | VALIDO ATÉ {valido_ate}"); st.rerun()

with tab2:
    st.markdown("### 🔄 ENTRADA E SAÍDA DE MATERIAL")
    if not st.session_state.get('lista_cadastro'): st.warning("Cadastre primeiro")
    else:
        c1,c2,c3,c4=st.columns(4)
        with c1: lote_mov=st.selectbox("LOTE*", options=[str(r.get('LOTE','')) for r in st.session_state.lista_cadastro if r.get('LOTE')], key="sel_lote_mov")
        with c2: tipo_mov=st.selectbox("TIPO*", ["SAIDA","ENTRADA"], key="sel_tipo_mov")
        with c3: qtd_mov=st.number_input("QTD KG*", value=100.0, min_value=1.0, key="num_qtd_mov")
        with c4: motivo=st.text_input("MOTIVO","REFORMA FORNO 2", key="txt_motivo")
        if st.button("✅ REGISTRAR ENTRADA/SAIDA", type="primary", use_container_width=True, key="btn_reg_mov"):
            st.session_state.lista_mov.append({"LOTE":str(lote_mov),"TIPO":tipo_mov,"QTD":float(qtd_mov),"MOTIVO":motivo,"DATA":date.today().strftime("%d/%m/%Y"),"HORA":datetime.now().strftime("%H:%M")})
...
with tab3:
    st.markdown("### 📦 ESTOQUE COMPLETO - TODAS AS INFORMAÇÕES")
    if not st.session_state.get('lista_cadastro'): st.warning("Sem cadastro")
        ...
        c3.markdown(f"<div class='card'><h4>📥 ENTRADAS</h4><h2 style='color:green;'>{sum([float(v.get('ENTRADAS',0) or 0) for v in saldos.values()]):,.0f}</h2></div>", unsafe_allow_html=True)
        c4.markdown(f"<div class='card'><h4>📤 SAIDAS</h4><h2 style='color:red;'>{sum([float(v.get('SAIDAS',0) or 0) for v in saldos.values()]):,.0f}</h2></div>", unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        html="""<table class="tabela"><tr><th>ID</th><th>DESCRIÇÃO</th><th>MARCA</th><th>LOTE</th><th>FABRICAÇÃO</th><th>TEMPO VALIDADE</th><th>VÁLIDO ATÉ</th><th>QTD/PAL</th><th>ENTRADA</th><th>TOTAL INIC</th><th>ENTRADAS</th><th>SAÍDAS</th><th>SALDO ATUAL</th><th>DATA CAD</th><th>STATUS</th></tr>"""
        for lote,r in saldos.items():
            saldo=float(r.get('SALDO',0) or 0)
            html+=f"""<tr><td><b>{r.get('ID','')}</b></td><td>{r.get('DESCRICAO','')}</td><td>{r.get('MARCA','')}</td><td class='lote'>{lote}</td><td class='fab'><b>{r.get('FABRICACAO','')}</b></td><td><b>{r.get('TEMPO_VALIDADE','')} M</b></td><td class='val'><b>{r.get('VALIDO_ATE','')}</b></td><td>{r.get('QTD_PALETE','')}</td><td>{r.get('ENTRADA','')}</td><td><b>{float(r.get('TOTAL',0) or 0):,.0f}</b></td><td style='background:#a0ffa0;'><b>{float(r.get('ENTRADAS',0) or 0):,.0f}</b></td><td style='background:#ffb0b0;'><b>{float(r.get('SAIDAS',0) or 0):,.0f}</b></td><td style='background:#7fff7f;'><b>{saldo:,.0f}</b></td><td>{r.get('DATA_CADASTRO','')}</td><td><b>{'✅ OK' if saldo>0 else '⛔ ZERADO'}</b></td></tr>"""
        html+="</table>"
...
with tab5:
    ...
    if not st.session_state.get('lista_cadastro'): st.warning("Sem dados")
        ...
        try:
            df=pd.DataFrame(st.session_state.lista_cadastro)
