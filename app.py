...
st.markdown("""<style>.tabela { width:100%; border-collapse:collapse; font-size:11px; display:block; overflow-x:auto; }.tabela th { background:#1a252f; color:#fff; padding:10px 5px; border:2px solid #000; text-align:center; font-family:Arial Black; font-size:10px; }.tabela td { padding:8px 5px; border:1.5px solid #000; text-align:center; background:#fff; color:#000; font-weight:700; font-size:11px; }.lote { background:#00ff66!important; border:3px solid #000!important; }.val { background:#ffff00!important; border:2px solid #000!important; }.fab { background:#a0d8ff!important; }.fundo-verde { background:#A8C5A2; border-left:6px solid #000; padding:20px 10px 20px 0; margin:15px 0; border:3px solid #000; }.barra { height:54px; margin:16px 0; border:2.5px solid #000; display:flex; align-items:center; padding-left:12px; font-family:Arial Black; font-size:11px; box-shadow:4px 4px 0 #000; }.azul { background:#6FA8DC; }.branca { background:#FFFFFF; }.cinza { background:#8A8A8A; color:#fff; }</style>""", unsafe_allow_html=True)
...
with tab1:
    with st.form("form_cadastro_principal", clear_on_submit=True):
        ...
        if st.form_submit_button("💾 CADASTRAR", type="primary", use_container_width=True):
            ...
            st.session_state.lista_cadastro.append({"ID":str(id_in).strip(),"DESCRICAO":str(desc_in).upper().strip(),"MARCA":str(marca_in).upper().strip(),"LOTE":str(lote_in).strip(),"FABRICACAO":fab_str,"TEMPO_VALIDADE":int(safe_float(tempo_in,12)),"VALIDO_ATE":valido_ate,"QTD_PALETE":safe_float(qtd_in),"ENTRADA":safe_float(ent_in),"TOTAL":total,"UNIDADE":str(unidade_in).upper(),"LOCAL":local_in,"DATA_CADASTRO":date.today().strftime("%d/%m/%Y")})
            pd.DataFrame(st.session_state.lista_cadastro).to_csv(ARQ_CAD,index=False)
            st.success(f"✅ LOTE {lote_in} em {local_in} - {ent_in:.1f} PALETES = {total:.0f} {unidade_in}"); st.rerun()

with tab2:
    st.markdown("### 🔄 MOVIMENTAÇÃO - COM AUTO PREENCHIMENTO DA ÚLTIMA VEZ")
        ...
        if st.button(f"✅ CONFIRMAR REGISTRO", type="primary", use_container_width=True, key="btn_reg_mov"):
            if tipo_mov=="SAIDA" and saldo_atual and safe_float(saldo_atual.get('SALDO_PALETES',0))<safe_float(paletes_mov):
                ...
                if local_mov==LOCAL_OFICINA and tipo_mov=="ENTRADA":
                    st.session_state.lista_mov.append({"LOTE":str(lote_mov),"TIPO":"SAIDA","PALETES":safe_float(paletes_mov),"QTD_POR_PALETE":safe_float(qtd_por_palete_base),"TOTAL_QTD":safe_float(total_qtd_mov),"UNIDADE":unidade_base,"MOTIVO":f"AUTO TRANSFER -> {motivo}","DATA":date.today().strftime("%d/%m/%Y"),"HORA":datetime.now(fuso).strftime("%H:%M"),"LOCAL_MOV":LOCAL_GALPAO,"OBS":f"TRANSFERÊNCIA AUTO"})
                    st.session_state.lista_mov.append({"LOTE":str(lote_mov),"TIPO":"ENTRADA","PALETES":safe_float(paletes_mov),"QTD_POR_PALETE":safe_float(qtd_por_palete_base),"TOTAL_QTD":safe_float(total_qtd_mov),"UNIDADE":unidade_base,"MOTIVO":motivo,"DATA":date.today().strftime("%d/%m/%Y"),"HORA":datetime.now(fuso).strftime("%H:%M"),"LOCAL_MOV":LOCAL_OFICINA,"OBS":f"TRANSFERÊNCIA AUTO"})
                elif local_mov==LOCAL_GALPAO and tipo_mov=="SAIDA":
                    st.session_state.lista_mov.append({"LOTE":str(lote_mov),"TIPO":"SAIDA","PALETES":safe_float(paletes_mov),"QTD_POR_PALETE":safe_float(qtd_por_palete_base),"TOTAL_QTD":safe_float(total_qtd_mov),"UNIDADE":unidade_base,"MOTIVO":f"AUTO TRANSFER -> {motivo}","DATA":date.today().strftime("%d/%m/%Y"),"HORA":datetime.now(fuso).strftime("%H:%M"),"LOCAL_MOV":LOCAL_GALPAO,"OBS":f"TRANSFERÊNCIA AUTO"})
                    st.session_state.lista_mov.append({"LOTE":str(lote_mov),"TIPO":"ENTRADA","PALETES":safe_float(paletes_mov),"QTD_POR_PALETE":safe_float(qtd_por_palete_base),"TOTAL_QTD":safe_float(total_qtd_mov),"UNIDADE":unidade_base,"MOTIVO":motivo,"DATA":date.today().strftime("%d/%m/%Y"),"HORA":datetime.now(fuso).strftime("%H:%M"),"LOCAL_MOV":LOCAL_OFICINA,"OBS":f"TRANSFERÊNCIA AUTO"})
                elif local_mov==LOCAL_OFICINA and tipo_mov=="SAIDA":
                    st.session_state.lista_mov.append({"LOTE":str(lote_mov),"TIPO":"SAIDA","PALETES":safe_float(paletes_mov),"QTD_POR_PALETE":safe_float(qtd_por_palete_base),"TOTAL_QTD":safe_float(total_qtd_mov),"UNIDADE":unidade_base,"MOTIVO":motivo,"DATA":date.today().strftime("%d/%m/%Y"),"HORA":datetime.now(fuso).strftime("%H:%M"),"LOCAL_MOV":LOCAL_OFICINA,"OBS":f"SAÍDA REAL"})
                else:
    ...
    if not st.session_state.get('lista_cadastro'):
        ...
        if not saldos:
            ...
            for chave,d in saldos.items():
                lista.append({"LOTE": str(d.get('LOTE_ORIG')),"DESCRICAO": f"ID {d.get('ID','?')} - {str(d.get('DESCRICAO','SEM DESC'))[:20]}","ID": str(d.get('ID','?')),"LOCAL": str(d.get('LOCAL')),"VALIDO_ATE": str(d.get('VALIDO_ATE','00/00/0000')),"UNIDADE": str(d.get('UNIDADE','KG')),"SALDO_QTD": safe_float(d.get('SALDO_QTD',0)),"SALDO_PAL": safe_float(d.get('SALDO_PALETES',0)),"TEXTO_QTD": f"{safe_float(d.get('SALDO_QTD',0)):,.0f} {str(d.get('UNIDADE','KG'))}","TEXTO_PAL": f"{safe_float(d.get('SALDO_PALETES',0)):.1f} PAL"})
            ...
            if df.empty:
                ...
                fig1=px.bar(df, x='DESCRICAO', y='SALDO_QTD', color='LOCAL', barmode="group", text='TEXTO_QTD', title="SALDO QTD POR PRODUTO - GALPÃO vs OFICINA")
                ...
                c1,c2=st.columns(2)
                with c1:
                    fig2=px.bar(df, x='LOTE', y='SALDO_PAL', color='LOCAL', text='TEXTO_PAL', title="SALDO EM PALETES POR LOTE")
                ...
                with c2:
                    ...
                    fig_pizza = px.pie(df_pizza, values='SALDO_QTD', names='LOCAL', title="DISTRIBUIÇÃO GALPÃO vs OFICINA", hole=0.3)
                ...
                fig3=px.bar(df_sorted, x='LOTE', y='SALDO_QTD', color='LOCAL', barmode="group", text='TEXTO_QTD', hover_data=["DESCRICAO","VALIDO_ATE","UNIDADE","SALDO_PAL"], title="SALDO QTD POR LOTE - TOTAL GERAL = GALPÃO + OFICINA")
