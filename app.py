...
with tab_est:
    ...
    if lista:
        ...
        df['TOTAL_ID']=df['ID'].apply(lambda x: totais.get(x,0))
...
with tab_graf:
    ...
    if not lista:
        ...
        df=pd.DataFrame(lista)
        df_g=df.groupby(['ID','DESCRICAO','MARCA'], as_index=False)['SALDO'].sum()
        ...
        if id_sel:
            ...
            st.write(f"ID {id_sel} - {desc_id}")
            for _, r in df_id.iterrows():
                st.write(f"Marca {r['MARCA']} = {r['SALDO']:,.0f}")
            st.write(f"Total ID {id_sel} = {total_id:,.0f}")

            # GRAFICO 1 - HORIZONTAL - MARCA - QTD - ID SELECIONADO
            fig=px.bar(
                df_id,
                x='SALDO',
                y='MARCA',
            ...
            fig2=px.bar(
                ...
                barmode='stack',
                orientation='h',
                title="Todos IDs - HORIZONTAL - Cor = MARCA - Comprimento = QTD"
            )
            fig2.update_traces(texttemplate='%{text:,.0f}', textposition='inside')
            fig2.update_layout(xaxis_title="QTD", yaxis_title="ID")
            st.plotly_chart(fig2, use_container_width=True)

            # GRAFICO 3 - HORIZONTAL - TODOS IDs - COR POR ID
            totais_id=df_g.groupby(['ID'], as_index=False)['SALDO'].sum()
            fig3=px.bar(
                totais_id,
                x='SALDO',
                y='ID',
                ...
                text='SALDO',
                orientation='h',
                title="TOTAL POR ID - HORIZONTAL - QTD"
            )
            fig3.update_traces(texttemplate='%{text:,.0f}', textposition='outside')
            fig3.update_layout(xaxis_title="QTD - TOTAL DO ID", yaxis_title="ID", showlegend=False)
            st.plotly_chart(fig3, use_container_width=True)

            df_show=df_id[['MARCA','SALDO']].copy()
            df_show.columns=['MARCA','QTD']
