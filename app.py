...
st.set_page_config(page_title="REFORMA DE FORNOS V10.1 FINAL", layout="wide", page_icon="🔥")
...
st.markdown(f"<h1 style='text-align:center; background:#000; color:#00ff66; padding:15px; border-radius:12px; border:4px solid #ff4e00;'>🔥 REFORMA DE FORNOS | {agora_br.strftime('%d/%m/%Y %H:%M:%S')} | FINAL 🔥</h1>", unsafe_allow_html=True)
...
with tab_dash:
    ...
    if df.empty:
        st.warning("Sem estoque - cadastre materiais")
    else:
        ...
        fig = px.bar(df_id, x='ID_DESC', y='SALDO_QTD', text='TEXTO', title="TOP 10 IDs - NUMEROS VISIVEIS")
        fig.update_traces(textposition='outside', textfont_size=16)
        fig.update_layout(height=500)
        st.plotly_chart(fig, use_container_width=True)
        df_local = df.groupby('LOCAL')[['SALDO_QTD']].sum().reset_index()
        fig2 = px.pie(df_local, values='SALDO_QTD', names='LOCAL', title="POR LOCAL - 3 LOCAIS", hole=0.4)
...
with tab1:
    st.subheader("📝 CADASTRO - ID reconhece tudo + EDITAR/EXCLUIR + MULTILOCAL")
    id_busca = st.text_input("🔍 DIGITE ID PARA RECONHECER TODOS OS MATERIAIS", key="cad_busca", placeholder="Ex: 1")
    ...
    if st.session_state.edit_idx is not None:
        ...
        if ult: desc_default=ult.get('DESCRICAO',''); marca_default=ult.get('MARCA',''); qtd_default=safe_float(ult.get('QTD_PALETE',1250),1250)
    with st.form("form_cad_final"):
        c1,c2=st.columns([2,1])
        with c1:
            id_in=st.text_input("ID* (categoria)", value=id_default)
            desc_in=st.text_input("DESCRIÇÃO* (mostra em tudo)", value=desc_default)
            marca_in=st.text_input("MARCA* (mesmo ID pode ter varias)", value=marca_default)
        ...
        with c4:
            un_in=st.selectbox("UNIDADE*", ["KG","UNIDADE","SACO","BLOCO","TIJOLO","LATA","CAIXA","METRO","LITRO"])
        ...
        b1,b2=st.columns(2)
        with b1: btn=st.form_submit_button(f"💾 CADASTRAR EM {len(locs)} LOCAL(IS)", type="primary", use_container_width=True)
        with b2: btn_c=st.form_submit_button("❌ CANCELAR EDICAO", use_container_width=True)
    ...
    for idx,reg in enumerate(st.session_state.lista_cadastro):
        if id_busca and id_busca.upper() not in str(reg.get('ID','')).upper() and id_busca.upper() not in str(reg.get('DESCRICAO','')).upper(): continue
        ...
        with c3:
            if st.button("EDITAR", key=f"e{idx}", use_container_width=True): st.session_state.edit_idx=idx; st.rerun()
        with c4:
            if st.button("EXCLUIR", key=f"d{idx}", type="primary", use_container_width=True): st.session_state.lista_cadastro.pop(idx); pd.DataFrame(st.session_state.lista_cadastro).to_csv(ARQ_CAD,index=False); st.rerun()
        with c5:
            msg=f"ID {reg.get('ID')} - {reg.get('DESCRICAO')} - {reg.get('LOCAL')}"
            st.link_button("ZAP", f"https://wa.me/?text={urllib.parse.quote(msg)}", use_container_width=True)
...
with tab2:
    ...
    if not ids: st.warning("Cadastre ID primeiro")
        ...
        id_busca_mov=st.text_input("🔍 DIGITE O ID", key="mov_busca")
        c1,c2=st.columns(2)
        with c1:
            ...
            lote=st.text_input("LOTE* - OBRIGATORIO AQUI")
        ...
        with c2:
            ...
            fab=st.date_input("FABRICAÇÃO LOTE (FIFO)", value=date.today())
        ...
        if st.button("✅ CONFIRMAR MOVIMENTACAO", type="primary", use_container_width=True):
...
with tab4:
    ...
    id_b=st.text_input("🔍 DIGITE ID", key="busca_id", placeholder="Ex: 1")
    if id_b:
        ...
        if itens:
            st.dataframe(pd.DataFrame([{"ID":r.get('ID'),"DESCRIÇÃO":r.get('DESCRICAO'),"MARCA":r.get('MARCA'),"LOTE":r.get('LOTE'),"LOCAL":r.get('LOCAL')} for _,r in itens]), use_container_width=True)
        if dados:
...
with tab5:
    ...
    if ids:
        ...
        with c2:
            ...
            resp=st.text_input("RESPONSÁVEL*", value="OPERADOR")
...
st.caption(f"V10.1 FINAL - TODAS ABAS FUNCIONANDO - {agora_br.strftime('%d/%m/%Y %H:%M:%S')}")
